...
const client = new WirePusher({ token: 'wpt_YOUR_TOKEN' });
...
**Get your token:** Open app → Settings → Teams → [Team Name] → copy token
...
- `token` (string, required): Your WirePusher team token (starts with `wpt_`; legacy user tokens start with `wpu_`)
